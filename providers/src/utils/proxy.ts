...
const DEFAULT_PROXY_URL = '/proxy/proxy';
// Default M3U8 proxy URL for HLS stream proxying
let CONFIGURED_M3U8_PROXY_URL = '/proxy/proxy.m3u8';
...
export function createM3U8ProxyUrl(url: string, features?: FeatureMap, headers: Record<string, string> = {}): string {
  ...
  return `${CONFIGURED_M3U8_PROXY_URL}/?type=hls&url=${encodedUrl}${headers ? `&headers=${encodedHeaders}` : ''}`;
}
...
export function updateM3U8ProxyUrl(url: string): string {
  if (url.includes('/proxy.m3u8?type=hls&url=')) {
    return url.replace(/https:\/\/[^/]+\/proxy\.m3u8/, CONFIGURED_M3U8_PROXY_URL);
  }
  return url;
}
